...
#if S7_HWID_L3L(S7, S7301, T0) || S7_HWID_L3H(S7, S7301, C)
#define PM8901_MPP_1 (0)
#define PM8901_MPP_3 (2)
static unsigned int gSpeakerEnFlag = 0;

static int config_class_d0_mpp(int enable)
{
	int rc;
...
int config_boost_5v_gpio(int enable)
{
	int rc = 0;
    static char vreg_5v_use_cnt = 0;
    ...
    pr_info("*********[%d][%d]**********\n", enable, vreg_5v_use_cnt);  /* debug use */

	if (enable) {
		class_d0_mpp.control = PM8XXX_MPP_DOUT_CTRL_HIGH;
		rc = pm8xxx_mpp_config(PM8901_MPP_PM_TO_SYS(PM8901_MPP_1),
			&class_d0_mpp);

		if (rc) {
			pr_err("%s: MPP1 enable failed\n", __func__);
			return rc;
		}
	    ...
	    vreg_5v_use_cnt--;
        if (vreg_5v_use_cnt == 0){            
    		class_d0_mpp.control = PM8XXX_MPP_DOUT_CTRL_LOW;
    		rc = pm8xxx_mpp_config(PM8901_MPP_PM_TO_SYS(PM8901_MPP_1),
			&class_d0_mpp);

    		if (rc) {
    			pr_err("%s: MPP1 enable disable failed\n", __func__);
                vreg_5v_use_cnt++;      /* if close fail, the cnt will be added 1 */
    			return rc;
    		}
		}
        if (vreg_5v_use_cnt < 0)
            vreg_5v_use_cnt = 0;        /* vreg_5v_use_cnt can not be negative */
	}
	return 0;
}
			 ...
			 const char *buf, size_t count)
{
    ...
    if(-1 == iret) {
		...
		rc = config_boost_5v_gpio(gSpeakerEnFlag);
		//msleep(10);
        rc |= config_class_d0_mpp(gSpeakerEnFlag);
...
/* Define the device attributes */
 static struct kobj_attribute speaker_attribute =
         __ATTR(switch, 0664, speaker_attr_show, speaker_attr_store);
...
#if S7_HWID_L3H(S7, S7301, C)
    mdelay(30);
//    pr_info("config_boost_5v_gpio(1)\n");  /* debug use */
	rc = config_boost_5v_gpio(1);
    mdelay(20);
//    pr_info("config_class_d0_mpp(1)\n");  /* debug use */
	rc |= config_class_d0_mpp(1);
	return rc;
#else
...
#endif
#else
	return enable_tpa2028d1_function();
#endif
}
...
{
#if defined(CONFIG_PMIC8058_OTHC) || defined(CONFIG_PMIC8058_OTHC_S7PRO)
	int ret = 0;
#ifndef CONFIG_BELASIGNA_BS300
