...
module_param(appsbark, int, S_IRUGO | S_IWUSR);
...
static struct workqueue_struct *dog_wq = NULL;

static void pet_watchdog_work(struct work_struct *work);
static void init_watchdog_work(struct work_struct *work);
static void wdog_appsbark_dump(unsigned long data);
static DECLARE_DELAYED_WORK(dogwork_struct, pet_watchdog_work);
static DECLARE_WORK(init_dogwork_struct, init_watchdog_work);
static DECLARE_TASKLET(wdog_dump, wdog_appsbark_dump, 0);
...
static void pet_watchdog_work(struct work_struct *work)
{
	...
	if (enable){
		...
		queue_delayed_work_on(0, dog_wq, &dogwork_struct, delay_time);
        }
}
...
static void init_watchdog_work(struct work_struct *work)
{
	...
	//schedule_delayed_work_on(0, &dogwork_struct, delay_time);
	dog_wq = create_singlethread_workqueue("msm_wdog_wq");
	if(!dog_wq){
		printk(KERN_ERR "create msm_wdog_wq thread failed\n");
		return;
        }
