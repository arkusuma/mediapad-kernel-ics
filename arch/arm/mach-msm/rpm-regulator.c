...
static int msm_rpm_vreg_debug_mask=0;
...
#if 0
	//This is a hack for the RPM_21XX, which causes the S7-Pro crash when the L3B is enabled
	//The case is being tracked by the QC SR#00513467
	if ((*(vreg)).id == RPM_VREG_ID_PM8901_L3)
	{
		printk(KERN_ERR "%s: skipping the vreg_set for %d\n", __func__, (*(vreg)).id);
		//return 0;
	}
#endif
