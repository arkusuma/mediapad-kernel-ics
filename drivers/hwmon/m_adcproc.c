...
static const struct adc_map_pt_S7 adbmap_ntcgtherm_S7[] = {
		{2157, -40000, 0},
		{2139, -35000, 0},
		{2115, -30000, 0},
		{2083, -25000, 0},
...
static int32_t
	adc_map_linear_S7(const struct adc_map_pt_S7 *pts,
		uint32_t tablesize, int32_t input, int64_t *output)
{
	...
	*output = (((int32_t) ((pts[i].y - pts[i-1].y)*
			(input - (pts[i-1].x - pts[i-1].c)))/
			((pts[i].x - pts[i].c) - (pts[i-1].x - pts[i-1].c)))+
			pts[i-1].y);
	}
     printk(KERN_EMERG "adc_map_linear: input = %d, output = %d\n", input,(int32_t) (*output));
     return 0;
}
