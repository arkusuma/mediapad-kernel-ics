...
//	if (!(reg & PM8058_RTC_ENABLE) || (reg & PM8058_RTC_ALARM_ENABLE)) {
//#else
...
//
    ...
    pm8058_rtc0_read_time(&pdev->dev, &default_time);
	if (default_time.tm_year == 70 && default_time.tm_mon == 0) {
		default_time.tm_year = 111;
        default_time.tm_mon  = 0;
        default_time.tm_mday = 1;
    }
    pm8058_rtc0_set_time(&pdev->dev, &default_time);
...
static int pm8058_rtc_resume(struct device *dev)
{
	...
	if (reg & PM8058_RTC_ALARM_ENABLE) {
		reg &= ~PM8058_RTC_ALARM_ENABLE;
		rc = pm8058_write(rtc_dd->pm_chip, PM8058_RTC_CTRL,
							&reg, 1);
		if (rc < 0) {
