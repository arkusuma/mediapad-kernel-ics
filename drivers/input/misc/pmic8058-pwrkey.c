...
static struct pmic8058_pwrkey *p_spwrkey = 0;
...
static ssize_t pmic8058_attr_show(struct device_driver *driver, char *buf)
{
	int iRet = 0;

	printk("pmic8058_read: p_spwrkey(0x%x)---wxh \n", (int)p_spwrkey);

	iRet = sprintf(buf, "0x%x", (int)p_spwrkey);

    	return iRet;

}

static ssize_t pmic8058_attr_store(struct device_driver *driver, const char *buf, size_t count)
{
	int iRet = 0;

	printk("pmic8058_sim_pwrkey: p_spwrkey(0x%x)---wxh \n", (int)p_spwrkey);
	...
	if(p_spwrkey)
	{
	    input_report_key(p_spwrkey->pwr, KEY_POWER, 1);
	    input_sync(p_spwrkey->pwr);
	    input_report_key(p_spwrkey->pwr, KEY_POWER, 0);
	    input_sync(p_spwrkey->pwr);
	}
...
static DRIVER_ATTR(simulate_pwrkey, 0664,  pmic8058_attr_show, pmic8058_attr_store);
...
static int __devinit pmic8058_pwrkey_probe(struct platform_device *pdev)
{
	...
	printk("pmic8058_pwrkey_probe: pwrkey(0x%x)---wxh \n", (int)pwrkey);
	
	err = driver_create_file(&(pmic8058_pwrkey_driver.driver), &driver_attr_simulate_pwrkey);
	if (0 != err )
	{
	    printk("failed to create sysfs entry(sim_pwrkey): %d\n", err);
	    return -1;
	}
