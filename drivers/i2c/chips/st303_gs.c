...
#ifdef CONFIG_HAS_EARLYSUSPEND
struct early_suspend lsm303dlh_acc_earlysuspend;
static void lsm303dlh_acc_early_suspend(struct early_suspend *h);
static void lsm303dlh_acc_late_resume(struct early_suspend *h);
#endif
...
static int lsm303dlh_acc_device_power_off(struct lsm303dlh_acc_data *acc)
{
        
	int err = 0;
	u8 buf[2] = { CTRL_REG4, LSM303DLH_ACC_PM_OFF };
	u8 config[2] = {CTRL_REG1, POWER_DOWN_MODE};
	err = lsm303dlh_acc_i2c_write(acc, buf, 1); 
	if (err < 0){
		dev_err(&acc->client->dev, "disabling sleep to wake failed\n");
	}
...
{
	int err;
	u8 config[2] = {CTRL_REG1, acc->resume_state[0]};    
	...
	err = lsm303dlh_acc_i2c_write(acc, config, 1);
	if (err < 0){
		dev_err(&acc->client->dev, "soft power on failed\n");
		return err;
	}
...
static int lsm303dlh_acc_disable(struct lsm303dlh_acc_data *acc)
{
	...
	if (atomic_cmpxchg(&acc->enabled, 1, 0)) {
		...
		err = lsm303dlh_acc_device_power_off(acc);
		if(err < 0){
...
static int lsm303dlh_acc_misc_close(struct inode *inode, struct file *file)
{
    int err= 0;
    atomic_dec(&open_num);   
    if ((atomic_read(&open_num) == 0) || (atomic_read(&open_num) < 0)) {
		err = lsm303dlh_acc_disable(lsm303dlh_acc_misc_data);	
		if(err < 0){
		    printk(KERN_ERR "%s,%d error",__func__,__LINE__);
            return err;
		}
...
static const struct file_operations lsm303dlh_acc_misc_fops = {
    ...
    /* End: leyihua modified for Linux Kernel 3.0, end 2011/11/26 */
    .release = lsm303dlh_acc_misc_close,
};
...
static int coordinate_acc = -1;
static ssize_t coordinate_acc_show(struct device_driver *drv, char *buf)
{
	return sprintf(buf, "coordinate_acc =  %d\n",coordinate_acc);
}

static ssize_t coordinate_acc_store(struct device_driver *drv, const char *buf, size_t count)
{
	int status;
	...
	if ((status != 0) && (status != 1)) {
    	...
    	coordinate_acc = status;
        if (coordinate_acc == 1) {
            lsm303dlh_acc_misc_data->pdata->axis_map_x = 0;
            lsm303dlh_acc_misc_data->pdata->axis_map_y = 1;
            lsm303dlh_acc_misc_data->pdata->axis_map_z = 2;
            
            lsm303dlh_acc_misc_data->pdata->negate_x = 0;
            lsm303dlh_acc_misc_data->pdata->negate_y = 1;
            lsm303dlh_acc_misc_data->pdata->negate_z = 1;
                
        } else if (coordinate_acc == 0) {
            lsm303dlh_acc_misc_data->pdata->axis_map_x = 1;
            lsm303dlh_acc_misc_data->pdata->axis_map_y = 0;
            lsm303dlh_acc_misc_data->pdata->axis_map_z = 2;
            
            lsm303dlh_acc_misc_data->pdata->negate_x = 0;
            lsm303dlh_acc_misc_data->pdata->negate_y = 0;
            lsm303dlh_acc_misc_data->pdata->negate_z = 1;
...
static DRIVER_ATTR(coordinate_acc, 0644, coordinate_acc_show, coordinate_acc_store);
...
static int lsm303dlh_acc_input_init(struct lsm303dlh_acc_data *acc)
{
	...
	if (sensor_dev == NULL) {
		...
		if (acc->input_dev == NULL) {
			err = -ENOMEM;
			printk(KERN_ERR "gs_probe: Failed to allocate input device\n");
			goto err0;
		}

	  acc->input_dev->name = "sensors"; 
	  sensor_dev = acc->input_dev;
	} else {
	  acc->input_dev = sensor_dev;
	}
    
	acc->input_dev->id.vendor = GS_ST35DE;//for  akm8973 compass detect.
#ifdef LSM303DLH_ACC_OPEN_ENABLE
			   ...
			   const struct i2c_device_id *id)					//?? be used for Device INIT
{
	...
	#ifdef CONFIG_HAS_EARLYSUSPEND
	lsm303dlh_acc_earlysuspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN;
	lsm303dlh_acc_earlysuspend.suspend = lsm303dlh_acc_early_suspend;
	lsm303dlh_acc_earlysuspend.resume = lsm303dlh_acc_late_resume;
    	register_early_suspend(&lsm303dlh_acc_earlysuspend);
	#endif
...
static int __devexit lsm303dlh_acc_remove(struct i2c_client *client)
{
	...
	#ifdef CONFIG_HAS_EARLYSUSPEND
    	unregister_early_suspend(&lsm303dlh_acc_earlysuspend);
	#endif
...
{
	int ret;
    ret = i2c_add_driver(&lsm303dlh_acc_driver);
    if (ret) {
		printk(KERN_ERR "[%s %d] add lsm303dlh_acc_driver failed", __func__, __LINE__);
        return ret;
    }

    ret = driver_create_file(&lsm303dlh_acc_driver.driver, &driver_attr_coordinate_acc);
    if (ret < 0) {
		printk(KERN_ERR "[%s %d] create lsm303dlh_acc_driver coordinate sys failed", __func__, __LINE__);
    }
	return 0;
}
