...
{
#if 0
	ldo9 = regulator_get(NULL, "8058_l9");//DVDD&&DOVDD
	if (IS_ERR(ldo9)) {
		pr_err("%s: VREG LDO9 get failed\n", __func__);
		ldo9 = NULL;
		return;
	}
	if (regulator_set_voltage(ldo9, 1800000, 1800000)) {
		pr_err("%s: VREG LDO9 set voltage failed\n",  __func__);
		goto ldo9_disable;
	}
#if 0
	if (regulator_enable(ldo9)) {
		pr_err("%s: VREG ldo9 enable failed\n", __func__);
		goto ldo9_put;
#endif
	if (regulator_is_enabled(ldo9) == 0) {
		if (regulator_enable(ldo9)) {
			pr_err("%s: VREG ldo9 enable failed\n", __func__);
			goto ldo9_put;
		}
		...
		CDBG("%s: VREG ldo9 a alrady enabled\n", __func__);
	}
	ldo15 = regulator_get(NULL, "8058_l15");//AVDD
	if (IS_ERR(ldo15)) {
...
#endif
#if S7_HWID_L3H(S7, S7301, C)
    pr_err("%s: WAC: Open AF powersupply \n", __func__);

	ldo19 = regulator_get(NULL, "8058_l19");//AF_VDD
	if (IS_ERR(ldo19)) {
		pr_err("%s: VREG LDO19 get failed\n", __func__);
		ldo19 = NULL;
		goto ldo19_disable;
	}
	if (regulator_set_voltage(ldo19, 2800000, 2800000)) {
		pr_err("%s: VREG LDO19 set voltage failed\n",  __func__);
		goto ldo19_disable;
	}
#if 0
	if (regulator_enable(ldo19)) {
		pr_err("%s: VREG LDO19 enable failed\n", __func__);
		goto ldo19_put;
#endif
	if (regulator_is_enabled(ldo19) == 0) {
		if (regulator_enable(ldo19)) {
			pr_err("%s: VREG LDO19 enable failed\n", __func__);
			goto ldo19_put;
		}
		...
		CDBG("%s: VREG LDO19 is already enabled\n", __func__);
	}
#endif
	...
	if (regulator_enable(ldo25)) {
		...
		goto ldo25_put;
#endif
	if (regulator_is_enabled(ldo25) == 0) {
		if (regulator_enable(ldo25)) {
			pr_err("%s: VREG LDO25 enable failed\n", __func__);
			goto ldo25_put;
		}
		...
		CDBG("%s: VREG LDO25 is already enabled\n", __func__);
	}
...
#endif
	} else if (regulator_is_enabled(fs_vfe) == 0) {
		if (regulator_enable(fs_vfe)) {
			CDBG("%s: Regulator FS_VFE enable failed\n", __func__);
			regulator_put(fs_vfe);
		}
	}
	else {
		CDBG("%s: Regulator FS_VFE is already enabled\n", __func__);
	}
	return;
...
ldo15_disable:
	...
	if (regulator_is_enabled(ldo15) > 0)
		regulator_disable(ldo15);
ldo15_put:
	regulator_put(ldo15);
#endif
...
#if 0
	//if (ldo15) {
	if ((ldo15) && (regulator_is_enabled(ldo15) > 0)) {
		regulator_disable(ldo15);
		regulator_put(ldo15);
	}
	//if (ldo9) {
	if ((ldo9) && (regulator_is_enabled(ldo9) > 0)) {
		regulator_disable(ldo9);
		regulator_put(ldo9);
	}
#endif
#if S7_HWID_L3H(S7, S7301, C)
	//if (ldo19) {
	if ((ldo19) && (regulator_is_enabled(ldo19) > 0)) {
		regulator_disable(ldo19);
		regulator_put(ldo19);
	}
#endif
	//if (ldo25) {
	if ((ldo25) && (regulator_is_enabled(ldo25) > 0)) {
		regulator_disable(ldo25);
		regulator_put(ldo25);
	}
	//if (fs_vfe) {
	if ((fs_vfe) && (regulator_is_enabled(fs_vfe) > 0)) {
...
EXPORT_SYMBOL(msm_camio_clk_rate_set);
...
EXPORT_SYMBOL(msm_camio_csi_config);
